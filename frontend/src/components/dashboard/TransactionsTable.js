...
const TransactionsTable = ({ transactions, onUpdate }) => {
  ...
  const confirmDelete = async () => {
    if (selectedTx) {
      await deleteExpense({ expenseId: selectedTx._id, userId: selectedTx.usersid });
    ...
    <div className="glass-panel overflow-hidden w-full flex flex-col">
      ...
      <div className="p-6 border-b border-white/10 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          >
            <option value="" className="bg-brand-900">All Categories</option>
            {categories.map(c => <option key={c} value={c} className="bg-brand-900">{c}</option>)}
          </select>
          ...
          >
            <option value="latest" className="bg-brand-900">Latest</option>
            <option value="oldest" className="bg-brand-900">Oldest</option>
            <option value="highest" className="bg-brand-900">Highest Amount</option>
            <option value="lowest" className="bg-brand-900">Lowest Amount</option>
          </select>
        </div>
      </div>
              ...
              paginatedData.map((exp) => (
                <tr key={exp._id} className="border-b border-white/5 hover:bg-white/5 transition-colors group">
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 rounded-full border text-xs font-medium ${getCategoryColor(exp.category)}`}>
                      {exp.category}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-slate-300 text-sm">{formatDate(exp.date)}</td>
      ...
      {totalPages > 1 && (
        <div className="p-4 border-t border-white/10 flex justify-between items-center bg-black/20">
          <p className="text-sm text-slate-400">
            Showing <span className="text-white font-medium">{(currentPage - 1) * itemsPerPage + 1}</span> to <span className="text-white font-medium">{Math.min(currentPage * itemsPerPage, processedData.length)}</span> of <span className="text-white font-medium">{processedData.length}</span> results
          </p>
      ...
      <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} title="Confirm Deletion">
        <div className="text-slate-300">
          <p className="mb-6">Are you sure you want to delete this {selectedTx?.category} expense of <span className="text-white font-bold tracking-wider font-handjet text-lg">₹ {selectedTx?.amount.toLocaleString()}</span>? This action cannot be undone.</p>
